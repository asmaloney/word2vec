...
static void InitNet( void )
{
    ...
    for ( a = 0; a < vocab_size; a++ )
    {
        for ( b = 0; b < layer1_size; b++ )
        {
            ...
            syn0[a * layer1_size + b] = ( ( ( next_random & 0xFFFF ) / (real)65536 ) - 0.5 ) /
                                        layer1_size;
        }
    }
...
int main( int argc, char **argv )
{
    ...
    if ( argc == 1 )
    {
        ...
        printf( "\t-sample <float>\n" );
        printf( "\t\tSet threshold for occurrence of words. Those that appear with higher "
                "frequency in the training data\n" );
        printf( "\t\twill be randomly down-sampled; default is 1e-3, useful range is (0, 1e-5)\n" );
        printf( "\t-hs <int>\n" );
        printf( "\t\tUse Hierarchical Softmax; default is 0 (not used)\n" );
        printf( "\t-negative <int>\n" );
        printf( "\t\tNumber of negative examples; default is 5, common values are 3 - 10 (0 = not "
                "used)\n" );
        printf( "\t-threads <int>\n" );
        ...
        printf( "\t-alpha <float>\n" );
        printf( "\t\tSet the starting learning rate; default is 0.025 for skip-gram and 0.05 for "
                "CBOW\n" );
        printf( "\t-classes <int>\n" );
        printf( "\t\tOutput word classes rather than word vectors; default number of classes is 0 "
                "(vectors are written)\n" );
        printf( "\t-debug <int>\n" );
        ...
        printf( "\t\tThe vocabulary will be read from <file>, not constructed from the training "
                "data\n" );
        printf( "\t-cbow <int>\n" );
        printf( "\t\tUse the continuous bag of words model; default is 1 (use 0 for skip-gram "
                "model)\n" );
        printf( "\nExamples:\n" );
        printf( "./word2vec -train data.txt -output vec.txt -size 200 -window 5 -sample 1e-4 "
