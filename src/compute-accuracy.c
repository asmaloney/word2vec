...
int main( int argc, char **argv )
{
    ...
    float *M = NULL;
    char *vocab;
    int TCN, CCN = 0, TACN = 0, CACN = 0, SECN = 0, SYCN = 0, SEAC = 0, SYAC = 0, QID = 0, TQ = 0,
             TQS = 0;
    if ( argc < 2 )
    {
        printf( "Usage: ./compute-accuracy <FILE> <threshold>\nwhere FILE contains word "
                "projections, and threshold is used to reduce vocabulary of the model for fast "
                "approximate evaluation (0 = off, otherwise typical value is 30000)\n" );
        return 0;
    }
    ...
    while ( 1 )
    {
        ...
        if ( ( !strcmp( st1, ":" ) ) || ( !strcmp( st1, "EXIT" ) ) || feof( stdin ) )
        {
            ...
            if ( QID != 0 )
            {
                ...
                printf( "Total accuracy: %.2f %%   Semantic accuracy: %.2f %%   Syntactic "
                        "accuracy: %.2f %% \n",
